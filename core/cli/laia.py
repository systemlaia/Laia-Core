...
from librarian.summarize import command_summarize
from librarian.classify import command_classify
from librarian.review import command_review
from librarian.approve import command_approve
from librarian.finalize import command_finalize
from librarian.catalog import command_catalog
from librarian.dedupe import command_dedupe
from librarian.failures import command_mark_failures
from librarian.pending import command_pending
from librarian.extract import command_extract
from librarian.export import command_export
...
def build_parser():
    ...
    workflow_scan_p = workflow_sub.add_parser("scan-document", help="Scan a document through human review")
    workflow_scan_p.add_argument("--profile", default="document")
    workflow_scan_p.add_argument("--project", default="Inbox")
    workflow_scan_p.add_argument("--tags", default="")
    workflow_scan_p.add_argument("--dry-run", action="store_true")
    workflow_scan_p.add_argument("--verbose", action="store_true")
    workflow_scan_p.set_defaults(func=command_scan_document)

    ingest_p = sub.add_parser("ingest", help="Media ingest commands")
    ingest_sub = ingest_p.add_subparsers(dest="subcommand")

    scan_p = ingest_sub.add_parser("scan", help="Scan documents into LAIA ingest packets")
    scan_p.add_argument("--test", action="store_true")
    scan_p.add_argument("--list-options", action="store_true")
    scan_p.add_argument("--profile", default="document")
    scan_p.add_argument("--project", default="Inbox")
    scan_p.add_argument("--tags", default="")
    scan_p.add_argument("--dry-run", action="store_true")
    scan_p.add_argument("--device", default=None)
    scan_p.set_defaults(func=command_scan)

    librarian_p = sub.add_parser("librarian", help="Librarian packet commands")
    librarian_sub = librarian_p.add_subparsers(dest="subcommand")

    librarian_index_p = librarian_sub.add_parser("index", help="Index ingest packets")
    librarian_index_p.add_argument("--last", action="store_true")
    librarian_index_p.set_defaults(func=command_index)

    librarian_route_p = librarian_sub.add_parser("route", help="Route indexed ingest packets")
    librarian_route_p.add_argument("--last", action="store_true")
    librarian_route_p.set_defaults(func=command_route)

    librarian_summarize_p = librarian_sub.add_parser("summarize", help="Summarize ingest packets")
    librarian_summarize_p.add_argument("--last", action="store_true")
    librarian_summarize_p.set_defaults(func=command_summarize)

    librarian_classify_p = librarian_sub.add_parser("classify", help="Classify summarized ingest packets")
    librarian_classify_p.add_argument("--last", action="store_true")
    librarian_classify_p.set_defaults(func=command_classify)

    librarian_review_p = librarian_sub.add_parser("review", help="Prepare human review for classified ingest packets")
    librarian_review_p.add_argument("--last", action="store_true")
    librarian_review_p.set_defaults(func=command_review)

    librarian_approve_p = librarian_sub.add_parser("approve", help="Approve reviewed ingest packets")
    librarian_approve_p.add_argument("--last", action="store_true")
    librarian_approve_p.set_defaults(func=command_approve)

    librarian_finalize_p = librarian_sub.add_parser("finalize", help="Finalize approved ingest packets")
    librarian_finalize_p.add_argument("--last", action="store_true")
    librarian_finalize_p.set_defaults(func=command_finalize)

    librarian_dedupe_p = librarian_sub.add_parser("dedupe", help="Detect candidate duplicate ingest packets")
    librarian_dedupe_p.add_argument("--last", action="store_true")
    librarian_dedupe_p.set_defaults(func=command_dedupe)

    librarian_extract_p = librarian_sub.add_parser("extract", help="Extract structured fields from finalized packets")
    librarian_extract_p.add_argument("--last", action="store_true")
    librarian_extract_p.add_argument("--project", default="")
    librarian_extract_p.add_argument("--category", default="")
    librarian_extract_p.add_argument("--limit", type=int, default=20)
    librarian_extract_p.add_argument("--force", action="store_true")
    librarian_extract_p.add_argument("--json", action="store_true")
    librarian_extract_p.set_defaults(func=command_extract)

    librarian_export_p = librarian_sub.add_parser("export", help="Export extracted catalog data")
    librarian_export_p.add_argument("--project", default="")
    librarian_export_p.add_argument("--category", default="")
    librarian_export_p.add_argument("--format", choices=["csv", "json"], default="csv")
    librarian_export_p.add_argument("--limit", type=int, default=None)
    librarian_export_p.add_argument("--output", default="")
    librarian_export_p.add_argument("--apply-corrections", action="store_true")
    librarian_export_p.add_argument("--raw", action="store_true")
    librarian_export_p.set_defaults(func=command_export)

    librarian_extract_report_p = librarian_sub.add_parser("extract-report", help="Report extraction quality")
    librarian_extract_report_p.add_argument("--project", default="")
    librarian_extract_report_p.add_argument("--category", default="")
    librarian_extract_report_p.add_argument("--limit", type=int, default=None)
    librarian_extract_report_p.add_argument("--json", action="store_true")
    librarian_extract_report_p.add_argument("--raw", action="store_true")
    librarian_extract_report_p.set_defaults(func=command_extract_report)

    librarian_correct_extract_p = librarian_sub.add_parser("correct-extract", help="Write human extraction corrections")
    librarian_correct_extract_p.add_argument("--packet", required=True)
    librarian_correct_extract_p.add_argument("--merchant")
    librarian_correct_extract_p.add_argument("--transaction-date", dest="transaction_date")
    librarian_correct_extract_p.add_argument("--transaction-time", dest="transaction_time")
    librarian_correct_extract_p.add_argument("--subtotal")
    librarian_correct_extract_p.add_argument("--tax")
    librarian_correct_extract_p.add_argument("--tip")
    librarian_correct_extract_p.add_argument("--total")
    librarian_correct_extract_p.add_argument("--payment-method", dest="payment_method")
    librarian_correct_extract_p.add_argument("--last-four", dest="last_four")
    librarian_correct_extract_p.add_argument("--currency")
    librarian_correct_extract_p.add_argument("--note")
    librarian_correct_extract_p.set_defaults(func=command_correct_extract)

    librarian_inspect_extract_p = librarian_sub.add_parser("inspect-extract", help="Inspect one extraction before correction")
    librarian_inspect_extract_p.add_argument("--packet", required=True)
    librarian_inspect_extract_p.add_argument("--lines", type=int, default=80)
    ...
    librarian_pending_p = librarian_sub.add_parser("pending", help="List pending review packets")
    librarian_pending_p.add_argument("--limit", type=int, default=20)
    librarian_pending_p.add_argument("--json", action="store_true")
    librarian_pending_p.set_defaults(func=command_pending)

    librarian_catalog_p = librarian_sub.add_parser("catalog", help="Read finalized ingest catalog entries")
    librarian_catalog_p.add_argument("--last", action="store_true")
    librarian_catalog_p.add_argument("--project", default="")
    librarian_catalog_p.add_argument("--category", default="")
    librarian_catalog_p.add_argument("--limit", type=int, default=20)
    librarian_catalog_p.add_argument("--json", action="store_true")
    librarian_catalog_p.set_defaults(func=command_catalog)

    grocy_p = sub.add_parser("grocy", help="Grocy operational state service")
    grocy_sub = grocy_p.add_subparsers(dest="subcommand")

    grocy_status_p = grocy_sub.add_parser("status", help="Check Grocy reachability")
    grocy_status_p.set_defaults(func=command_grocy_status)

    grocy_checkins_p = grocy_sub.add_parser("checkins", help="Grocy check-in planning")
    grocy_checkins_sub = grocy_checkins_p.add_subparsers(dest="checkins_command")
    grocy_checkins_draft_p = grocy_checkins_sub.add_parser("draft", help="Draft LAIA Grocy check-ins")
    grocy_checkins_draft_p.set_defaults(func=command_grocy_checkins_draft)
...
def main():
    ...
    if args.command == "briefing":
        ...
        command_scan(args)
    elif args.command == "librarian" and args.subcommand == "index":
        command_index(args)
    elif args.command == "librarian" and args.subcommand == "route":
        command_route(args)
    elif args.command == "librarian" and args.subcommand == "summarize":
        command_summarize(args)
    elif args.command == "librarian" and args.subcommand == "classify":
        command_classify(args)
    elif args.command == "librarian" and args.subcommand == "review":
        command_review(args)
    elif args.command == "librarian" and args.subcommand == "approve":
        command_approve(args)
    elif args.command == "librarian" and args.subcommand == "finalize":
        command_finalize(args)
    elif args.command == "librarian" and args.subcommand == "dedupe":
        command_dedupe(args)
    elif args.command == "librarian" and args.subcommand == "extract":
        command_extract(args)
    elif args.command == "librarian" and args.subcommand == "export":
        command_export(args)
    elif args.command == "librarian" and args.subcommand == "extract-report":
        command_extract_report(args)
    elif args.command == "librarian" and args.subcommand == "correct-extract":
    ...
    elif args.command == "librarian" and args.subcommand == "catalog":
        command_catalog(args)
    elif args.command == "grocy" and args.subcommand == "status":
        command_grocy_status(args)
    elif args.command == "grocy" and args.subcommand == "checkins" and args.checkins_command == "draft":
        command_grocy_checkins_draft(args)
